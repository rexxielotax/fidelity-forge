import { a as getRequest, n as createMiddleware } from "./server-DzloFYEC.mjs";
import { t as createClient } from "../_libs/supabase__supabase-js.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/auth-middleware-D13lCX2a.js
...
var requireSupabaseAuth = createMiddleware({ type: "function" }).client(async ({ next }) => {
	const { supabase } = await import("../_libs/_2.mjs").then((n) => n.t);
	const { data } = await supabase.auth.getSession();
	const token = data.session?.access_token;
	return next({ headers: { Authorization: token ? `Bearer ${token}` : "" } });
}).server(async ({ next }) => {
