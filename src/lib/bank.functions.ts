...
export const createTransfer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: account, error: accErr } =
      await supabaseAdmin
        .from("accounts")
    ...
    await supabaseAdmin.from("notifications").insert({
      ...
      message: `Your transfer of $${data.amount.toFixed(
        2
      )} to ${data.recipientName} is pending.`,
    });

    return tx;
  });

export const finalizeTransfer = createServerFn({
  method: "POST",
})
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (data: { transactionId: string }) => data
  )
  .handler(async ({ data, context }) => {
    const { supabaseAdmin } = await import(
      "@/integrations/supabase/client.server"
    ...
    await supabaseAdmin
      ...
      .insert({
        ...
        message: `$${Number(tx.amount).toFixed(
          2
        )} sent to ${tx.recipient_name}. Ref ${tx.reference}.`,
      });
...
const DEPOSIT_METHOD_LABEL: Record<DepositInput["method"], string> = {
...
export const createDeposit = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: DepositInput) => {
    ...
    if (!DEPOSIT_METHOD_LABEL[data.method]) {
      throw new Error("Select a deposit method");
    }
...
export const requestCard = createServerFn({
  method: "POST",
})
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (data: {
      ...
      if (!TIER_FEES[data.cardType]) {
        throw new Error("Unknown card type");
      }

      if (!data.accountId) {
        throw new Error("Select a funding account");
      }
  ...
  .handler(async ({ data, context }) => {
    ...
    if (Number(account.balance) < tier.fee) {
      throw new Error(
        `Insufficient balance. The processing fee is $${tier.fee.toLocaleString()}.`
      );
    }
    ...
    await supabaseAdmin
      ...
      .update({
        balance:
          Number(account.balance) - tier.fee,
      })
      .eq("id", account.id);
...
export const setCardStatus = createServerFn({
  method: "POST",
})
  .middleware([requireSupabaseAuth])
  .inputValidator(
...
export const getCardPin = createServerFn({
  method: "POST",
})
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (data: { cardId: string }) => data
  )
  .handler(async ({ data, context }) => {
    const { supabaseAdmin } = await import(
      "@/integrations/supabase/client.server"
...
export const notifyEvent = createServerFn({
  method: "POST",
})
  .middleware([requireSupabaseAuth])
  .inputValidator(
