...
export const Route = createFileRoute("/_authenticated/deposit")({
  head: () => ({
    meta: [
      { title: "Deposit — wellsfargo Bank" },
      { name: "description", content: "Add funds to your account using a supported deposit method." },
      { property: "og:title", content: "Deposit — wellsfargo Bank" },
      { property: "og:description", content: "Add funds to your account." },
    ],
  }),
...
const METHODS = [
  { id: "paypal", label: "PayPal", icon: Wallet, hint: "Deposit via PayPal" },
  { id: "cashapp", label: "Cash App", icon: CreditCard, hint: "Deposit via Cash App" },
  { id: "bank_transfer", label: "Bank Transfer", icon: Landmark, hint: "Deposit via bank transfer" },
  { id: "crypto", label: "Crypto", icon: Bitcoin, hint: "Bitcoin & USDT" },
] as const;
...
const METHOD_INFO: Record<MethodId, string> = {
  paypal:
    "Use the PayPal account details below to complete your deposit. This deposit is processed within wellsfargo Bank and does not initiate an external PayPal transaction.",
  cashapp:
    "Use the Cash App details below. This deposit is processed within wellsfargo Bank and does not connect to Cash App.",
  bank_transfer:
    "Use the bank transfer details below. This deposit is processed within wellsfargo Bank and does not initiate an external bank transfer.",
  crypto:
    "Choose Bitcoin or USDT below. This deposit is processed within wellsfargo Bank and does not interact with a blockchain network.",
};
...
const DEPOSIT_DETAILS = {
  paypal: {
    ...
    fields: [
      { label: "PayPal account", value: "deposits@wellsfargobank.io" },
      { label: "Account name", value: "wellsfargo Bank" },
    ],
    notice: "This account is provided for reference only and is not a live receiving account.",
  },
  cashapp: {
    title: "Cash App Deposit Details",
    description: "Use the Cash App details below.",
    fields: [
      { label: "Cash App", value: "$wellsfargoBank" },
      { label: "Account name", value: "wellsfargo Bank" },
    ],
    notice: "This Cash App identifier is provided for reference only and is not connected to a live account.",
  },
  bank_transfer: {
    title: "Bank Transfer Details",
    description: "Use these banking details when making your transfer.",
    fields: [
      { label: "Bank name", value: "wellsfargo Bank" },
      { label: "Account name", value: "wellsfargo Bank Account" },
      { label: "Account number", value: "0000000000" },
      { label: "Routing number", value: "000000000" },
      { label: "SWIFT / BIC", value: "NIRMGB2LXXX" },
    ],
    notice: "These banking details are provided for reference only and cannot receive real funds.",
  },
  usdt: {
    title: "USDT Deposit",
    description: "Select the network and use the deposit address below.",
    fields: [
      { label: "Network", value: "TRC20" },
      { label: "USDT address", value: "TNRML0000USDT0TRC200ADDRESS0000" },
    ],
    notice: "This is not a live blockchain address. Do not send real cryptocurrency to it.",
  },
  btc: {
    title: "Bitcoin Deposit",
    description: "Use the Bitcoin deposit address below.",
    fields: [
      { label: "Network", value: "Bitcoin" },
      { label: "BTC address", value: "bc1qwellsfargobank0000000000000000000" },
    ],
    notice: "This is not a live Bitcoin address. Do not send real cryptocurrency to it.",
  },
} as const;
...
function DepositPage() {
  ...
  const activeDepositMethod: SelectedMethod | "" = form.method === "crypto" ? form.cryptoMethod : form.method;

  const settingsRows = settingsQuery.data ?? [];

  const dynamicFields = activeDepositMethod
    ? settingsRows
        .filter((row) => row.method === activeDepositMethod)
        .map((row) => ({ label: row.field_label, value: row.field_value }))
    : [];

  const dynamicNotice = activeDepositMethod
    ? settingsRows.find((row) => row.method === activeDepositMethod && row.notice)?.notice
    : null;

  const staticDetails =
    activeDepositMethod && activeDepositMethod in DEPOSIT_DETAILS
      ? DEPOSIT_DETAILS[activeDepositMethod as keyof typeof DEPOSIT_DETAILS]
      : null;
  ...
  function chooseMethod(method: MethodId) {
    setErrorBanner(null);
    set({ method, cryptoMethod: "" });
  }

  function chooseCrypto(method: CryptoMethod) {
    setErrorBanner(null);
    set({ method: "crypto", cryptoMethod: method });
  }
  ...
  async function confirm() {
    ...
    try {
      const created = await send({
        data: {
          accountId,
          method: activeDepositMethod as "paypal" | "cashapp" | "bank_transfer" | "usdt" | "btc",
          amount,
        },
      });
      ...
      setTimeout(async () => {
        try {
           ...
           * Admin can resolve it from the admin panel.
           */
        } finally {
          setSettling(false);
        }
  ...
  return (
    <AppShell title="Deposit">
      ...
      {/* STEP 1 — METHOD */}
      {step === 0 && (
        <form onSubmit={goReview} className="space-y-6">
              ...
              {(accounts ?? []).map((a) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs font-medium">{money(Number(a.balance), currency)}</p>
                  </button>
                );
              })}
            </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">Choose how you want to add funds.</p>
            </div>
          ...
          {form.method === "crypto" && (
            <div className="rounded-2xl border border-border/70 bg-card p-4">
              <div className="mb-3">
                <p className="text-sm font-semibold">Choose cryptocurrency</p>
                <p className="text-xs text-muted-foreground">Select the asset you want to use.</p>
              </div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <button
                  ...
                  className={`rounded-xl border p-4 text-left transition ${
                    selectedCrypto === "usdt" ? "border-primary bg-primary/5" : "border-border/70 hover:bg-muted"
                  }`}
                >
                ...
                <button
                  ...
                  className={`rounded-xl border p-4 text-left transition ${
                    selectedCrypto === "btc" ? "border-primary bg-primary/5" : "border-border/70 hover:bg-muted"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className="grid size-10 place-items-center rounded-full bg-muted">
            ...
            <div className="rounded-2xl border border-border/70 bg-card p-5">
              <div className="mb-5">
                <div className="flex items-center gap-2">
                  <div className="grid size-9 place-items-center rounded-full bg-primary/10 text-primary">
                    {activeDepositMethod === "btc" ? (
                      <Bitcoin className="size-5" />
                    ) : activeDepositMethod === "usdt" ? (
                      <span className="font-bold">₮</span>
                    ) : activeDepositMethod === "bank_transfer" ? (
                  ...
                  <div>
                    <h3 className="text-base font-semibold">{selectedDetails.title}</h3>
              ...
              <div className="space-y-3">
                {selectedDetails.fields.map((field) => (
                  <div key={field.label} className="rounded-xl border border-border/60 bg-muted/40 p-3">
                    ...
                    <p className="break-all text-sm font-semibold">{field.value}</p>
                  </div>
                ))}
              </div>

              <div className="mt-4 rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3">
                <p className="text-xs leading-5 text-muted-foreground">{selectedDetails.notice}</p>
              </div>
            </div>
          )}
      ...
      {step === 1 && (
        <div className="space-y-5">
            ...
            <dl className="mt-5 space-y-3 text-sm">
              ...
              <div className="flex justify-between gap-4">
                ...
                <dd className="text-right font-medium">
                  {form.method === "crypto" ? (form.cryptoMethod === "usdt" ? "USDT" : "Bitcoin") : selectedMethod?.label ?? "—"}
                </dd>
              </div>

              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Amount</dt>
                <dd className="text-right font-medium">{money(Number(form.amount), currency)}</dd>
              </div>

              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Balance after settlement</dt>
                <dd className="text-right font-medium">
                  {money(Number(account?.balance ?? 0) + Number(form.amount || 0), currency)}
                </dd>
              </div>
            </dl>
          </div>
          ...
          {selectedDetails && (
            <div className="rounded-2xl border border-border/70 bg-card p-5">
              <p className="text-sm font-semibold">Deposit destination</p>
              <div className="mt-3 space-y-2">
                {selectedDetails.fields.map((field) => (
                  ...
                  >
                    <span className="text-xs text-muted-foreground">{field.label}</span>
                    <span className="break-all text-sm font-medium sm:text-right">{field.value}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-border/70 bg-card p-5">
            ...
            <div className="text-center">
              <h2 className="font-display text-xl font-bold">Confirm your deposit</h2>
              <p className="mt-1 text-sm text-muted-foreground">Review the information below before submitting.</p>
            </div>

            <div className="mt-5 rounded-xl bg-muted p-4">
              <div className="flex justify-between gap-4">
                <span className="text-sm text-muted-foreground">Amount</span>
                <span className="text-sm font-bold">{money(Number(form.amount), currency)}</span>
              </div>
          ...
          <div className="rounded-xl bg-muted px-4 py-3 text-sm text-muted-foreground">
            This deposit is processed within wellsfargo Bank. No funds are transferred through PayPal, Cash App, a bank network, or a cryptocurrency network.
          </div>
      ...
      {step === 3 && tx && settling && (
        <div className="space-y-5 text-center">
          ...
          <div>
            <h2 className="font-display text-xl font-bold">Processing your deposit</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Reference {tx.reference} · <StatusBadge status="pending" />
            </p>
            <p className="mt-2 text-xs text-muted-foreground">This usually takes a few seconds.</p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-5 text-center">
          ...
          <div>
            <h2 className="font-display text-xl font-bold">
              {tx.status === "completed" ? "Deposit complete" : "Deposit submitted"}
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
