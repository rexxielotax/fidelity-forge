...
const TX_FILTERS = ["all", "deposit", "transfer", "card_fee", "admin_adjustment"] as const;
...
function AdminDashboard() {
  ...
  const setTransferLock = useServerFn(adminSetTransferLock);
  ...
  const stats = useMemo(() => {
    ...
    const pendingDeposits = transactions.filter(
      (transaction) => transaction.status === "pending" && transaction.category === "deposit",
    ).length;

    const pendingTransfers = transactions.filter(
      (transaction) => transaction.status === "pending" && transaction.category === "transfer",
    ).length;
  ...
  async function runAction(id: string, fn: () => Promise<unknown>, successMessage: string) {
    if (actionId) return;
  ...
  async function handleCredit() {
    ...
    setCreditForm({ userId: "", accountType: "checking", amount: "", note: "" });
  }
      ...
      <main className="min-h-screen bg-muted/40">
        <div className="mx-auto flex min-h-screen max-w-6xl items-center justify-center px-4">
          <div className="w-full max-w-md rounded-2xl border border-border/70 bg-card p-8 text-center shadow-sm">
            ...
            <h1 className="mt-4 font-display text-lg font-bold">Loading admin console</h1>
  ...
  if (query.isError && !data) {
    const errorMessage =
      query.error instanceof Error ? query.error.message : "Unable to load the admin dashboard.";
    ...
    return (
      <main className="min-h-screen bg-muted/40">
        ...
        <Button onClick={() => navigate({ to: "/admin/deposit-settings" })}>
          Deposit Settings
        </Button>
          ...
          <div className="rounded-2xl border border-destructive/30 bg-card p-6">
            <div className="flex gap-3">
              ...
              <div className="min-w-0">
                <h2 className="font-display text-lg font-bold">Unable to load admin data</h2>
                <p className="mt-2 text-sm text-muted-foreground">
                  The admin data request failed. The dashboard has not redirected automatically.
                </p>
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        ...
        {query.isError && data && (
          <div className="mb-4 flex items-start gap-3 rounded-xl border border-destructive/30 bg-destructive/5 p-4">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold">Latest refresh failed</p>
              <p className="mt-1 break-words text-xs text-muted-foreground">
                {query.error instanceof Error ? query.error.message : "Unable to refresh admin data."}
              </p>
            </div>
            <Button className="ml-auto shrink-0" size="sm" variant="secondary" onClick={() => query.refetch()}>
              Retry
            </Button>
          </div>
        )}
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <StatCard label="Total balances" value={money(stats.balance, "USD")} />
          <StatCard label="Pending deposits" value={String(stats.pendingDeposits)} />
          <StatCard label="Pending transfers" value={String(stats.pendingTransfers)} />
          <StatCard label="Open tickets" value={String(stats.tickets)} />
        </div>
        ...
        <Tabs defaultValue="users" className="mt-6">
          ...
          <TabsContent value="users" className="space-y-4">
            ...
            <section className="rounded-2xl border border-border/70 bg-card p-5">
              <div>
                <h2 className="font-display text-base font-semibold">Credit an account</h2>
                <p className="mt-1 text-sm text-muted-foreground">
                  Add funds to a user's checking or savings account.
                </p>
              </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div className="space-y-2 lg:col-span-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setCreditForm((current) => ({ ...current, userId: value }))
                    }
                  >
                    <SelectTrigger>
                ...
                <div className="flex items-end">
                  <Button
                    ...
                    disabled={
                      ...
                      !creditForm.userId ||
                      !creditForm.amount ||
                      !Number.isFinite(Number(creditForm.amount)) ||
                      Number(creditForm.amount) <= 0
                    }
                    onClick={handleCredit}
                  >
                    {actionId === "credit-account" ? (
                      <>
            ...
            <section className="overflow-hidden rounded-2xl border border-border/70 bg-card">
                  ...
                  <tbody>
                    {(data?.profiles ?? []).map((profile) => {
                      const balance = (data?.accounts ?? [])
                        .filter((account) => account.user_id === profile.id)
                      ...
                      return (
                        <tr key={profile.id} className="border-t border-border/60">
                          ...
                          <td className="px-4 py-3">{money(balance, profile.currency || "USD")}</td>

                          <td className="px-4 py-3">
                            <Switch
                              checked={!profile.transfers_locked}
                              disabled={!!actionId}
                              onCheckedChange={(checked) =>
                                runAction(
                                  ...
                                  () =>
                                    setTransferLock({
                                      data: { userId: profile.id, locked: !checked },
                                    }),
                                  checked ? "Transfers enabled" : "Transfers disabled",
                                )
                              }
                            />
                          </td>
              ...
              {(data?.profiles ?? []).length === 0 && <EmptyTable message="No users found." />}
            </section>
          </TabsContent>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted/60 text-left text-xs uppercase tracking-wide text-muted-foreground">
                    <tr>
                      ...
                      <th className="px-4 py-3">Recipient / Description</th>
                    ...
                    {filteredTransactions.map((transaction) => (
                      <tr key={transaction.id} className="border-t border-border/60">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          {money(Number(transaction.amount ?? 0), "USD")}
                        </td>
          ...
          <TabsContent value="tickets" className="space-y-3">
            {(data?.tickets ?? []).map((ticket) => {
              ...
              return (
                <section key={ticket.id} className="rounded-2xl border border-border/70 bg-card p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-sm font-semibold">{ticket.subject}</p>
                      <p className="mt-1 text-xs capitalize text-muted-foreground">
                        {ticket.category} · {dateTime(ticket.created_at)}
                      </p>
                    </div>
                  ...
                  {ticket.admin_reply && (
                    <div className="mt-3 rounded-xl border border-primary/20 bg-primary/5 p-3">
                      <p className="text-xs font-semibold uppercase tracking-wide">Admin Reply</p>
                      <p className="mt-1 text-sm whitespace-pre-wrap">{ticket.admin_reply}</p>
                    </div>
                  )}
                    ...
                    <Button
                      ...
                      onClick={() =>
                        runAction(
                          ...
                          () =>
                            replyTicket({
                              data: { ticketId: ticket.id, reply: reply.trim(), status: "in_progress" },
                            }),
                          "Reply sent",
                        )
                      }
          ...
          <TabsContent value="audit">
            <section className="space-y-2">
              {(data?.actions ?? []).map((action) => (
                <div key={action.id} className="rounded-xl border border-border/70 bg-card px-4 py-3 text-sm">
                  <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <span className="font-medium">{action.action}</span>
                      <span className="text-muted-foreground"> by {action.admin_email}</span>
                    </div>
                    <span className="text-xs text-muted-foreground">{dateTime(action.created_at)}</span>
                  </div>
                </div>
              ))}
